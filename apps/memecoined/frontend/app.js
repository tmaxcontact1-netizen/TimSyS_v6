...
function renderTradingProfiles() {
  ...
  for (const profile of tradingProfiles) {
    ...
    const temporalFacts = Number(performance?.short_horizon_signals ?? 0) > 0
      ? `<div><dt>Short-horizon signals</dt><dd>${Number(performance.short_horizon_signals).toLocaleString()}</dd></div><div><dt>Signals passing all gates</dt><dd>${Number(performance.short_horizon_qualified ?? 0).toLocaleString()}</dd></div>`
      : "";
