...
function renderTradingProfiles() {
  ...
  for (const profile of tradingProfiles) {
    ...
    const technical = performance?.latest_signal?.technical;
    const technicalFacts = technical
      ? `<div><dt>Mathematical entry quality</dt><dd>${Number(technical.qualityScore)}/100</dd></div><div><dt>RSI</dt><dd>${Number(technical.rsi).toFixed(1)} ${technical.overextended ? "· overextended" : ""}</dd></div><div><dt>EMA alignment</dt><dd>${Number(technical.emaSpreadBps) / 100}% spread · ${Number(technical.emaSlopeBps) / 100}% slope</dd></div><div><dt>MACD confirmation</dt><dd>${Number(technical.macdHistogramBps) / 100}% histogram</dd></div><div><dt>Trend efficiency</dt><dd>${(Number(technical.efficiencyRatio) * 100).toFixed(1)}%</dd></div><div><dt>Whole-window return</dt><dd>${Number(technical.historyReturnBps) / 100}%</dd></div><div><dt>Largest observed drawdown</dt><dd>${Number(technical.maximumDrawdownBps) / 100}%</dd></div><div><dt>Recovery from observed low</dt><dd>${Number(technical.recoveryFromLowBps) / 100}%</dd></div><div><dt>Volatility-adjusted movement</dt><dd>${Number(technical.atrBps) / 100}% observed range</dd></div><div><dt>Executable quote bars</dt><dd>${Number(technical.barCount)} five-minute bars · ${Number(technical.sampleCount)} samples</dd></div><div><dt>Latest bar</dt><dd>${technical.bullishClose ? "Bullish close" : "No bullish close"}${technical.higherLows ? " · higher lows" : " · lows are not rising"}</dd></div>`
      : "";
    const calibrationFacts = calibration
      ...
      : "";
    const oscillator = performance?.latest_signal?.oscillation;
    const oscillatorFacts = oscillator
      ? `<div><dt>Oscillation score</dt><dd>${Number(oscillator.score)}/100</dd></div><div><dt>30-minute observations</dt><dd>${Number(oscillator.observationCount)}</dd></div><div><dt>SMA crossings</dt><dd>${Number(oscillator.smaCrossings)}</dd></div><div><dt>Return autocorrelation</dt><dd>${Number(oscillator.lagOneAutocorrelation).toFixed(3)}</dd></div><div><dt>Bollinger width</dt><dd>${(Number(oscillator.bollingerWidthBps) / 100).toFixed(2)}%</dd></div><div><dt>RSI</dt><dd>${Number(oscillator.currentRsi).toFixed(1)}</dd></div><div><dt>Distance from mean</dt><dd>${Number(oscillator.zScore).toFixed(2)}σ</dd></div><div><dt>Current decision</dt><dd>${escapeHtml(oscillator.reason)}</dd></div>`
      : "";
    const closedSignals = Number(performance?.telemetry_closed ?? 0);
    const validationFacts = Number(performance?.telemetry_signals ?? 0) > 0
      ? `<div><dt>Signals retained for validation</dt><dd>${Number(performance.telemetry_signals).toLocaleString()}</dd></div><div><dt>Signal-led entries</dt><dd>${Number(performance.telemetry_entered ?? 0).toLocaleString()}</dd></div><div><dt>Completed validation trades</dt><dd>${closedSignals.toLocaleString()}</dd></div><div><dt>Validated win rate</dt><dd>${closedSignals === 0 ? "Waiting for closed trades" : `${(100 * Number(performance.telemetry_wins ?? 0) / closedSignals).toFixed(1)}%`}</dd></div><div><dt>Average net outcome</dt><dd>${performance.average_net_bps == null ? "Waiting for closed trades" : `${(Number(performance.average_net_bps) / 100).toFixed(2)}%`}</dd></div><div><dt>Average measured friction</dt><dd>${performance.average_realized_friction_bps == null ? "Waiting for closed trades" : `${(Number(performance.average_realized_friction_bps) / 100).toFixed(2)}%`}</dd></div><div><dt>Rejected signals later profitable</dt><dd>${Number(performance.profitable_rejections ?? 0).toLocaleString()}</dd></div>`
      : "";
    const temporalFacts = Number(performance?.market_observations ?? 0) > 0
      ? `<div><dt>Live price samples</dt><dd>${Number(performance.market_observations).toLocaleString()}</dd></div><div><dt>Tokens being followed</dt><dd>${Number(performance.monitored_tokens ?? 0).toLocaleString()}</dd></div><div><dt>Tokens ready for pattern detection</dt><dd>${Number(performance.history_ready_tokens ?? 0).toLocaleString()}</dd></div><div><dt>Pattern checks completed</dt><dd>${Number(performance.short_horizon_signals ?? 0).toLocaleString()}</dd></div><div><dt>Adaptive calibrations</dt><dd>${Number(performance.adaptive_calibrations ?? 0).toLocaleString()}</dd></div><div><dt>Tradeable calibrated ranges</dt><dd>${Number(performance.adaptive_tradeable ?? 0).toLocaleString()}</dd></div><div><dt>Waiting for this strategy’s entry shape</dt><dd>${Number(performance.adaptive_pattern_waiting ?? 0).toLocaleString()}</dd></div><div><dt>Signals passing all gates</dt><dd>${Number(performance.short_horizon_qualified ?? 0).toLocaleString()}</dd></div>${performance.latest_signal ? `<div><dt>Latest detected pattern</dt><dd>${String(performance.latest_signal.pattern ?? "none").replaceAll("_", " ")}</dd></div><div><dt>Latest short movement</dt><dd>${Number(performance.latest_signal.shortMoveBps ?? 0) / 100}%</dd></div><div><dt>Five-minute volume change</dt><dd>${performance.latest_signal.volumeChangeBps == null ? "Unavailable" : `${Number(performance.latest_signal.volumeChangeBps) / 100}%`}</dd></div><div><dt>Liquidity change</dt><dd>${performance.latest_signal.liquidityChangeBps == null ? "Unavailable" : `${Number(performance.latest_signal.liquidityChangeBps) / 100}%`}</dd></div><div><dt>Buyer share</dt><dd>${performance.latest_signal.buyPressureBps == null ? "Unavailable" : `${Number(performance.latest_signal.buyPressureBps) / 100}%`}</dd></div>${technicalFacts}${calibrationFacts}` : ""}`
      : "";
    facts.innerHTML = `<div><dt>Risk per trade</dt><dd>${percentFromBps(profile.riskPerTradeBps)}</dd></div><div><dt>Maximum positions</dt><dd>${profile.maximumConcurrentPositions}</dd></div><div><dt>Typical time limit</dt><dd>${profile.maximumHoldingMinutes < 1440 ? `${profile.maximumHoldingMinutes} minutes` : `${profile.maximumHoldingMinutes / 1440} day(s)`}</dd></div><div><dt>Current-engine assessment runs</dt><dd>${assessed.toLocaleString()}</dd></div><div><dt>Different tokens assessed</dt><dd>${uniqueTokens.toLocaleString()}</dd></div><div><dt>Actionable signals now</dt><dd>${qualified.toLocaleString()}</dd></div>${temporalFacts}${oscillatorFacts}${validationFacts}<div><dt>Entry attempts made</dt><dd>${Number(performance?.entries_attempted ?? 0).toLocaleString()}</dd></div><div><dt>Entry attempts waiting</dt><dd>${Number(performance?.entries_pending ?? 0).toLocaleString()}</dd></div><div><dt>Entries rejected at execution</dt><dd>${Number(performance?.entries_failed ?? 0).toLocaleString()}</dd></div><div><dt>Entries filled</dt><dd>${Number(performance?.entries_entered ?? 0).toLocaleString()}</dd></div><div><dt>Current-engine buy and sell fills</dt><dd>${performance?.fills ?? 0}</dd></div><div><dt>Current-engine net result</dt><dd class="${result >= 0n ? "positive" : "negative"}">${performance ? signedSol(result) : "Waiting for first cycle"}</dd></div><div><dt>Return on assigned funds</dt><dd class="${result >= 0n ? "positive" : "negative"}">${performance ? percentage(result, initial, 2) : "—"}</dd></div><div><dt>Positions open now</dt><dd>${performance?.open_positions ?? 0}</dd></div><div><dt>Archived fills from older engines</dt><dd>${Math.max(0, Number(performance?.lifetime_fills ?? 0) - Number(performance?.fills ?? 0)).toLocaleString()}</dd></div>`;
