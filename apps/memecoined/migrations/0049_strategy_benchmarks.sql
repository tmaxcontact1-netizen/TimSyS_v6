...
ALTER TABLE paper_profile_activations
  ADD CONSTRAINT paper_profile_activations_profile_id_check CHECK (profile_id IN (
    ...
    'recovery_reversal','launch_transition','scalper','benchmark_buy_hold',
...
SELECT accounts.wallet, profiles.profile_id, false, 'observe', 10000, 1, now(), now()
FROM paper_accounts accounts
CROSS JOIN (VALUES
  ('benchmark_buy_hold'),('benchmark_momentum'),('benchmark_ema_cross'),
  ('benchmark_rsi_reversal'),('benchmark_macd_trend'),('benchmark_bollinger_reversion'),
  ('benchmark_donchian_breakout'),('benchmark_volume_breakout'),('benchmark_atr_trend')
) profiles(profile_id)
