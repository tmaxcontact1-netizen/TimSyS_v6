...
`paper_profile_activations` is the durable, versioned operator choice. Every mutation is authenticated, origin checked, allocation checked and append-only audited. A profile setting never grants live signing authority.
...
| `/frontend`           | Dependency-free local paper dashboard assets, including responsive navigation, browser-local display preferences, persistent watchlist CRUD, inert trading-configuration CRUD, safe legacy-list import, and portfolio views |
...
| `/src/infrastructure/database/dashboard-watchlists.ts`             | TypeScript | Provides wallet-scoped persistent watchlist CRUD with optimistic versions and atomic audit records                         | Database pool; dashboard watchlist schema                     | Local dashboard entrypoint                     | PostgreSQL client                     |
| `/src/infrastructure/database/dashboard-trading-configurations.ts` | TypeScript | Provides wallet-scoped inert trading-configuration CRUD with bounded values, optimistic versions, and atomic audit records | Database pool; dashboard configuration schema                 | Local dashboard entrypoint                     | PostgreSQL client                     |
...
| `/src/entrypoints/dashboard.ts`         | TypeScript | Serves local paper observations plus authenticated preference mutations and guarded paper-only cancellation/close requests; holds no live authority | Runtime config; database readiness; paper reporting and dashboard persistence | Operator-started local dashboard process | Node.js native HTTP server                       |
...
| `/tests/e2e/observation.test.ts`                 | TypeScript | Live-shaped discovery normalization, persistence boundary and provider refusal      | Discovery source/worker with fixture ports             | Test runner | Vitest                       |
| `/tests/e2e/shadow.test.ts`                      | TypeScript | Deterministic candidate decision with stale-evidence refusal and no execution       | Candidate evaluation work with fixture authority       | Test runner | Vitest                       |
| `/tests/e2e/paper.test.ts`                       | TypeScript | Approved paper work through fresh executable quote to simulated durable fill        | Paper execution with fixture quote/ledger/queue        | Test runner | Vitest                       |
| `/tests/e2e/failure-recovery.test.ts`            | TypeScript | Acquisition interruption, startup lease recovery and protected breaker behaviour    | Acquisition scheduler, supervisor and breaker domain   | Test runner | Vitest                       |
| `/tests/e2e/live-low-value.test.ts`              | TypeScript | Proves the future manual mainnet harness is denied by default and hard-value capped | Low-value trial authorization gate                     | Test runner | Vitest; no network or signer |
