...
function StudentProfileWidget() {
  ...
  }, [page, searchTerm, gradeFilter, homeroomFilter]);
  ...
  const grades = [
    ...new Set(students.map((s) => s.current_grade_level).filter(Boolean)),
  ].sort();
  const homerooms = [
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className="text-white">{student.enrollment_status}</span>
            </div>
            <div>
              <span className="text-gray-500">Grade:</span>{" "}
              <span className="text-white">{student.current_grade_level}</span>
            </div>
            <div>
              <span className="text-gray-500">Homeroom:</span>{" "}
              <span className="text-white">{student.homeroom}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="text-white">{student.primary_language}</span>
            </div>
          </div>

          {(student.medical_alert_flag ||
            student.special_education_flag ||
            student.gifted_talented_flag) && (
        ...
        {extended && (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              {extended.interests &&
                Object.keys(extended.interests).length > 0 && (
                  <div>
                    <strong className="text-gray-400">Interests:</strong>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Object.entries(extended.interests).map(([k, v]) =>
                        v ? (
                          <span
                            key={k}
              ...
              {extended.strengths &&
                Object.keys(extended.strengths).length > 0 && (
                  <div>
                    <strong className="text-gray-400">Strengths:</strong>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Object.entries(extended.strengths).map(([k, v]) =>
                        v ? (
                          <span
                            key={k}
                            className="bg-green-900/50 text-green-300 px-2 py-0.5 rounded"
                          >
                            {k}
                          </span>
                        ) : null,
              ...
              {extended.goals && extended.goals.length > 0 && (
                <div>
                  <strong className="text-gray-400">Goals:</strong>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <select
            ...
            onChange={(e) => {
              setHomeroomFilter(e.target.value);
              setPage(1);
            }}
            className="bg-gray-800 border border-gray-700 text-white rounded px-3 py-2 focus:outline-none focus:border-timsys-primary"
          >
