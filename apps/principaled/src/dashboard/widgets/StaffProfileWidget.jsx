        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="text-white">{s.staff_id}</span>
            </div>
            <div>
              <span className="text-gray-500">Job Title:</span>{" "}
              <span className="text-white">{s.job_title}</span>
            </div>
            <div>
              <span className="text-gray-500">Department:</span>{" "}
              <span className="text-white">{s.department}</span>
            </div>
            <div>
              <span className="text-gray-500">Status:</span>{" "}
              <span className="text-white">{s.employment_status}</span>
            </div>
            <div>
              <span className="text-gray-500">Type:</span>{" "}
              <span className="text-white">{s.employment_type}</span>
            </div>
            <div>
              <span className="text-gray-500">Hire Date:</span>{" "}
              <span className="text-white">{s.hire_date}</span>
            </div>
          </div>
          ...
          <div className="mt-4 flex gap-2 flex-wrap">
            <span
              className={`px-2 py-1 rounded text-xs ${
                s.dbs_check_status === "clear"
                  ? "bg-green-900/50 text-green-300"
                  : s.dbs_check_status === "pending"
                    ? "bg-yellow-900/50 text-yellow-300"
                    : s.dbs_check_status === "expired"
                      ? "bg-red-900/50 text-red-300"
                      : "bg-orange-900/50 text-orange-300"
              }`}
            >
              DBS: {s.dbs_check_status}
            </span>
            {s.dbs_expiry_date && (
              <span className="bg-gray-800 text-gray-400 px-2 py-1 rounded text-xs">
                Expires: {s.dbs_expiry_date}
              </span>
            )}
          </div>
        </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
            ...
            <div className="space-y-4 text-sm">
              {extended.career_goals && (
                <div>
                  <strong className="text-gray-400">Career Goals:</strong>{" "}
                  <p className="text-gray-300 mt-1">{extended.career_goals}</p>
                </div>
              )}
              {extended.professional_development &&
                extended.professional_development.length > 0 && (
                  <div>
                    ...
                    <ul className="mt-1 space-y-1">
                      {extended.professional_development.map((pd, idx) => (
                        <li
                          key={idx}
                          className="text-gray-300 bg-gray-800 rounded px-2 py-1"
                        >
                          {typeof pd === "string"
                            ? pd
                            : pd.title || JSON.stringify(pd)}
                        </li>
                      ))}
                    </ul>
                  </div>
                )}
              {extended.mentorship_roles &&
                extended.mentorship_roles.length > 0 && (
                  <div>
                    <strong className="text-gray-400">Mentorship Roles:</strong>
                    <ul className="mt-1 space-y-1">
                      {extended.mentorship_roles.map((role, idx) => (
                        ...
                        >
                          {typeof role === "string"
                            ? role
                            : role.title || JSON.stringify(role)}
                        </li>
                      ))}
                    </ul>
                  </div>
                )}
              {extended.committee_memberships &&
                extended.committee_memberships.length > 0 && (
                  <div>
                    <strong className="text-gray-400">
        ...
        {deep_insights && deep_insights.length > 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
            <h4 className="text-lg font-semibold text-purple-400 mb-4">
              Deep Insights ({deep_insights.length})
            </h4>
        ...
        {filteredStaff.length === 0 ? (
          ...
          <table className="w-full text-left">
            <thead className="bg-gray-800 border-b border-gray-700">
              <tr>
                <th className="w-16 px-4 py-2 text-sm font-semibold text-gray-500">
                  #
                </th>
                    ...
                    <span
                      className={`px-2 py-0.5 rounded text-xs ${
                        member.employment_status === "active"
                          ? "bg-green-900/50 text-green-300"
                          : member.employment_status === "leave"
                            ? "bg-yellow-900/50 text-yellow-300"
                            : "bg-gray-700 text-gray-300"
                      }`}
                    >
