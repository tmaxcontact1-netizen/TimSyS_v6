...
export function createDressedServer(input: {
  ...
  return createServer(async (request: IncomingMessage, response: ServerResponse) => {
    ...
    try {
      ...
      if (pathname === "/api/ensembles/catalogue") { if(method!=="GET")return json(response,405,{error:"method_not_allowed"});const[catalogue,garments,saved]=await Promise.all([styling.catalogue(),ensembles.candidates(),ensembles.saved()]);return json(response,200,{contexts:catalogue.contexts,garments:garments.map((garment)=>({id:garment.id,name:garment.name,slotId:garment.slotId,imageId:garment.imageId,categorySlug:garment.categorySlug})),saved}); }
      if (pathname === "/api/ensembles/generate") { if(method!=="POST")return json(response,405,{error:"method_not_allowed"});const requestValue=z.object({selectedGarmentId:z.string().uuid(),contextId:z.string().uuid(),season:z.enum(["spring","summer","autumn","winter"]).optional().nullable(),limitPerGrade:z.number().int().min(1).max(50).default(20)}).strict().parse(await body(request));const catalogue=await styling.catalogue(),context=catalogue.contexts.find((value)=>value.id===requestValue.contextId);if(!context)return json(response,400,{error:"invalid_context"});const[available,requirements,parameters,overrides]=await Promise.all([ensembles.candidates(),ensembles.requirements(context.id),styling.parameters(catalogue.ruleSet.id),ensembles.overrides()]),selected=available.find((garment)=>garment.id===requestValue.selectedGarmentId);if(!selected)return json(response,404,{error:"selected_garment_unavailable_or_unanalysed"});const results=generateEnsembles({selected,available,requirements,context,parameters,thresholds:catalogue.ruleSet.thresholds,season:requestValue.season??undefined,overrides,limitPerGrade:requestValue.limitPerGrade});const serialise=(items:typeof results.a)=>items.map((candidate)=>({key:candidate.key,garments:candidate.garments.map((garment)=>({id:garment.id,name:garment.name,slotId:garment.slotId,imageId:garment.imageId})),evaluation:candidate.evaluation}));return json(response,200,{a:serialise(results.a),b:serialise(results.b),c:serialise(results.c),examined:results.examined,rejected:results.rejected}); }
      if (pathname === "/api/ensembles/save") { if(method!=="POST")return json(response,405,{error:"method_not_allowed"});const requestValue=z.object({name:z.string().trim().min(1).max(120),garmentIds:z.array(z.string().uuid()).min(2).max(20),contextId:z.string().uuid(),season:z.enum(["spring","summer","autumn","winter"]).optional().nullable(),notes:z.string().trim().max(2000).optional().nullable()}).strict().parse(await body(request));const catalogue=await styling.catalogue(),context=catalogue.contexts.find((value)=>value.id===requestValue.contextId);if(!context)return json(response,400,{error:"invalid_context"});const available=await ensembles.candidates(),garments=requestValue.garmentIds.map((id)=>available.find((garment)=>garment.id===id)).filter((value)=>value!==undefined);if(garments.length!==requestValue.garmentIds.length)return json(response,404,{error:"garment_unavailable_or_unanalysed"});const parameters=await styling.parameters(catalogue.ruleSet.id),evaluation=evaluateStyling({garments,context,parameters,season:requestValue.season??undefined,thresholds:catalogue.ruleSet.thresholds});if(!evaluation.eligible||!evaluation.grade)return json(response,409,{error:"ensemble_not_saveable",evaluation});const evaluationId=randomUUID(),timestamp=now().toISOString();await styling.save({id:evaluationId,ruleSetId:catalogue.ruleSet.id,context,season:requestValue.season??null,garments,evaluation,timestamp});return json(response,201,await ensembles.saveOutfit({id:randomUUID(),name:requestValue.name,contextId:context.id,evaluationId,garments,evaluation,notes:requestValue.notes??null,timestamp})); }
