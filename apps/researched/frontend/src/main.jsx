      ...
      <main>
          ...
          <section>
            ...
            {!active || !analysis ? (
              ...
              <>
                ...
                {runResults&&<><h3>Generated results</h3><p>{["queued","running","paused"].includes(runResults.status)?`Background progress: ${runResults.progress_completed} of ${runResults.progress_total}. Results appear here as they complete.`:"Every result remains reviewable. Accepting or rejecting it records your reason in the audit trail."}</p><div className="source-actions export-actions"><button type="button" className="secondary" onClick={()=>exportAnalysisRun("markdown")}>Readable report</button><button type="button" className="secondary" onClick={()=>exportAnalysisRun("csv")}>Spreadsheet</button><button type="button" className="secondary" onClick={()=>exportAnalysisRun("json")}>Archive JSON</button></div>{runResults.items?.some(item=>item.last_error)&&<div className="notice">Some tasks need attention: {[...new Set(runResults.items.filter(item=>item.last_error).map(item=>String(item.last_error).replaceAll("_"," ")))].join("; ")}</div>}<div className="captured-list result-cards">{runResults.results?.map(result=><article key={result.id}><span>{labelize(result.analysis_type)} · {result.source_id?"single source":"cross-source corpus"} · {result.method} · {result.status}</span><ResultView result={result}/><small>{result.evidence_segment_ids?.length??0} traceable source sections · validated output contract v1</small>{result.status==="generated"&&<div className="source-actions"><button onClick={()=>reviewAnalysisResult(result,"accepted")}>Accept</button><button className="danger" onClick={()=>reviewAnalysisResult(result,"rejected")}>Reject</button></div>}</article>)}{!runResults.results?.length&&<div className="empty">No results have completed yet.</div>}</div></>}
        ...
        {tab === "reports" && (
          <section>
            ...
            <p>
              Generate a fixed, fingerprinted research output from the current
              study records. Later edits do not alter an existing report run.
            </p>
            {!active ? (
              <div className="empty">Select a study first.</div>
            ) : (
