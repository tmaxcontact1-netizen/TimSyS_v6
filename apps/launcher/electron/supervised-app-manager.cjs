...
function assertManifest(value, manifestPath) {
  if (
    ...
    value.kind !== "supervised-child"
  ) {
    throw new Error(`Unsupported supervised-app manifest: ${manifestPath}`);
  }
  if (
    !value.id ||
    !value.applicationRootEnvironment ||
    !value.processes ||
    Object.keys(value.processes).length === 0
  ) {
...
class SupervisedAppManager extends EventEmitter {
  ...
  async load(manifestPath) {
    ...
    const manifest = assertManifest(
      JSON.parse(await this.readManifest(absoluteManifestPath, "utf8")),
    ...
    if (existing && !TERMINAL_STATES.has(existing.state))
      throw new Error(`${manifest.id} is already managed`);
    const record = {
      id: manifest.id,
      manifest,
      applicationRoot,
  ...
  async start(manifestPath, extraEnvironment = {}) {
    ...
    const environment = {
      ...record.manifest.environmentDefaults,
      ...process.env,
      ...extraEnvironment,
    ...
    const workingDirectory = path.resolve(
      record.applicationRoot,
      record.manifest.workingDirectory || ".",
    );
    try {
      for (const [name, specification] of Object.entries(
        record.manifest.processes,
      )) {
        const command =
          specification.command === "node"
  ...
  async waitUntilHealthy(record, environment) {
    ...
    while (Date.now() < deadline) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.healthIntervalMilliseconds),
      );
    }
  ...
  beginRuntimeHealthMonitoring(record) {
    ...
    record.healthTimer = setInterval(async () => {
      if (!["running", "degraded"].includes(record.state)) return;
      try {
        ...
        if (record.healthFailures >= 3 && record.state !== "degraded") {
          this.transition(
            record,
            "degraded",
  ...
  async stop(appId) {
    ...
    record.stopPromise = this.stopProcesses(record).finally(() => {
  ...
  status(appId) {
    return this.snapshot(this.requireRecord(appId));
  }
  ...
  dashboardUrl(appId, environment) {
    ...
    const url = new URL(
      interpolateEnvironment(
        healthUrl,
        environment || record.environment || process.env,
      ),
    );
    url.pathname = "/";
    url.search = "";
  ...
  healthSpecification(record) {
    return (
      Object.values(record.manifest.processes).find((process) => process.health)
        ?.health || null
    );
  }
  ...
  snapshot(record) {
    return {
      ...
      processes: [...record.processes.keys()],
