...
function StudentsWidget({ students, onImport, onAdd, onEdit, onDelete, onWithdraw, onReinstate }) {
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                <td className="px-4 py-3 text-white text-sm">{r.student_id}</td>
                <td className="px-4 py-3 text-white text-sm">{r.first_name} {r.last_name}</td>
                <td className="px-4 py-3 text-gray-400 text-sm">{r.date_of_birth}</td>
                <td className="px-4 py-3 text-gray-400 text-sm">{r.sex}</td>
                <td className="px-4 py-3 text-sm">
                  <span className={`px-2 py-0.5 rounded text-xs ${r.enrollment_status === 'active' ? 'bg-green-900/50 text-green-300' : r.enrollment_status === 'withdrawn' ? 'bg-red-900/50 text-red-300' : r.enrollment_status === 'suspended' ? 'bg-yellow-900/50 text-yellow-300' : 'bg-gray-700 text-gray-300'}`}>{r.enrollment_status}</span>
                </td>
            ...
            {filtered.length === 0 && <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500">No students found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
