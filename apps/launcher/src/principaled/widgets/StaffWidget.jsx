...
function StaffWidget({ staff, onImport, onAdd, onEdit, onDelete, onWithdraw, onReinstate }) {
  const [search, setSearch] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [showImport, setShowImport] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [formData, setFormData] = useState({
    staff_id: '', first_name: '', last_name: '', hire_date: '', employment_type: 'full_time', job_title: '', employment_status: 'active',
  ...
  const handleEditClick = (r) => {
    setEditingId(r.id);
    setFormData({
      staff_id: r.staff_id || '', first_name: r.first_name || '', last_name: r.last_name || '', hire_date: r.hire_date || '', employment_type: r.employment_type || 'full_time', job_title: r.job_title || '', employment_status: r.employment_status || 'active',
  ...
  const handleWithdraw = async (id) => {
    ...
    if (!result.success) alert('Withdraw failed: ' + result.error);
  };

  const handleReinstate = async (id) => {
    const result = await onReinstate(id);
    if (!result.success) alert('Reinstate failed: ' + result.error);
  };
      ...
      {showForm && (
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><label className="block text-gray-400 text-xs mb-1">Employment Status</label><select value={formData.employment_status} onChange={(e) => setFormData({...formData, employment_status: e.target.value})} className="w-full bg-gray-800 border border-gray-700 rounded px-3 py-2 text-white text-sm focus:outline-none focus:border-timsys-primary"><option value="active">Active</option><option value="terminated">Terminated</option><option value="leave">Leave</option><option value="contract">Contract</option></select></div>
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.id} className="border-b border-gray-800 hover:bg-gray-800/50">
                <td className="px-4 py-3 text-white text-sm">{r.staff_id}</td>
                <td className="px-4 py-3 text-white text-sm">{r.first_name} {r.last_name}</td>
                <td className="px-4 py-3 text-gray-400 text-sm">{r.job_title || '—'}</td>
                <td className="px-4 py-3 text-gray-400 text-sm">{r.employment_type || '—'}</td>
                <td className="px-4 py-3 text-sm">
                  <span className={`px-2 py-0.5 rounded text-xs ${r.employment_status === 'active' ? 'bg-green-900/50 text-green-300' : r.employment_status === 'terminated' ? 'bg-red-900/50 text-red-300' : r.employment_status === 'leave' ? 'bg-yellow-900/50 text-yellow-300' : 'bg-gray-700 text-gray-300'}`}>{r.employment_status || '—'}</span>
                </td>
                <td className="px-4 py-3 text-right space-x-2 whitespace-nowrap">
                  <button onClick={() => handleEditClick(r)} className="text-timsys-primary hover:text-white text-sm">Edit</button>
                  {r.employment_status === 'active' ? (
                    <button onClick={() => handleWithdraw(r.id)} className="text-yellow-400 hover:text-yellow-300 text-sm">Withdraw</button>
                  ) : r.employment_status === 'terminated' ? (
                    <button onClick={() => handleReinstate(r.id)} className="text-green-400 hover:text-green-300 text-sm">Reinstate</button>
                  ) : null}
                  <button onClick={() => handleDelete(r.id)} className="text-red-400 hover:text-red-300 text-sm">Delete</button>
                </td>
              </tr>
            ))}
            {filtered.length === 0 && <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500">No staff found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
