...
export default function UpdatePanel() {
  ...
  const install = async () => {
    if (!window.confirm('Install this verified update now? TimSyS will close and reopen automatically.')) return;
    setState(current => ({ ...current, checking: true, error: '' }));
    try { await window.electronAPI.updates.install(); }
    catch (error) { setState(current => ({ ...current, checking: false, error: error.message })); }
  };
  ...
  return <section className="mt-10 rounded-lg border border-gray-800 bg-gray-900 p-5" aria-live="polite">
    ...
    {state.result?.updateAvailable && <div className="mt-4">
      ...
      <button type="button" disabled={state.checking} onClick={install} className="mt-4 rounded bg-timsys-primary px-5 py-2 text-sm font-semibold text-white disabled:opacity-50">{state.checking ? 'Installing…' : 'Install update'}</button>
    </div>}
  </section>;
}
