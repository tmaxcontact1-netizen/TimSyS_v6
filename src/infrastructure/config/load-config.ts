...
const databaseUrl = z.string().url().refine((value) => value.startsWith("postgres://") || value.startsWith("postgresql://"), "must be a PostgreSQL URI");
...
export interface RuntimeConfig {
  ...
  readonly solana: null | Readonly<{ primaryRpcUrl: string; fallbackRpcUrl: string; cluster: "mainnet-beta" | "devnet" }>;
  readonly execution: null | Readonly<{ heliusApiKey: string; jupiterApiKey: string; walletSecretFile: string }>;
}

const liveReads = new Set<OperatingMode>(["observation", "shadow", "paper", "supervised_live", "limited_auto", "full_auto"]);
...
export function loadRuntimeConfig(environment: NodeJS.ProcessEnv): RuntimeConfig {
  ...
  if (!parsed.success) throw new Error(`Invalid runtime configuration: ${z.prettifyError(parsed.error)}`);
  ...
  const execution = liveExecution.has(value.MEMECOINED_MODE)
    ...
    : null;
  return Object.freeze({ environment: value.MEMECOINED_ENV, mode: value.MEMECOINED_MODE, instanceId: value.MEMECOINED_INSTANCE_ID, logLevel: value.MEMECOINED_LOG_LEVEL, configDirectory: value.MEMECOINED_CONFIG_DIR, databaseUrl: value.DATABASE_URL, solana, execution });
}
