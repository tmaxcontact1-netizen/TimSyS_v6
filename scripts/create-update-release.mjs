...
const outputDirectoryArgument = process.argv.find((value) =>
  value.startsWith("--output-dir="),
);
const outputDirectoryName =
  outputDirectoryArgument?.slice("--output-dir=".length) ?? null;
if (
  outputDirectoryName !== null &&
  !/^[a-z0-9][a-z0-9-]{0,79}$/i.test(outputDirectoryName)
) {
  throw new Error(
    "--output-dir must be a simple folder name under dist-updates",
  );
}
const output =
  outputDirectoryName === null
    ? join(root, "dist-updates")
    : join(root, "dist-updates", outputDirectoryName);
const releaseVersion = process.argv[2];
const onlyArgument = process.argv.find((value) => value.startsWith("--only="));
const selectedBundles = onlyArgument
...
if (selectedBundles) {
  for (const id of selectedBundles)
    if (!Object.prototype.hasOwnProperty.call(sources, id))
      throw new Error(`Unknown bundle in --only: ${id}`);
}
...
async function createArchive(source, archive) {
  await new Promise((resolveRun, reject) => {
    const child = spawn("tar.exe", ["-a", "-cf", archive, "-C", source, "."], {
      windowsHide: true,
      stdio: "inherit",
    });
...
async function runNodeScript(script, arguments_ = []) {
  await new Promise((resolveRun, reject) => {
    const child = spawn(
      process.execPath,
      [join(root, "scripts", script), ...arguments_],
      {
        cwd: root,
        windowsHide: true,
        stdio: "inherit",
...
async function verifyMemecoinedPipeline() {
  if (!process.env.MEMECOINED_VERIFY_ADMIN_URL) {
    throw new Error(
      "MemeCoin'Ed update requires MEMECOINED_VERIFY_ADMIN_URL for its isolated full-path verification",
    );
  }
    ...
    const output = await new Promise((resolveRun, reject) => {
      ...
      const child = spawn(
        ...
        [
          join(
            ...
            "dist",
            "scripts",
            "diagnose-profile-roundtrips.js",
          ),
        ],
        {
          cwd: join(root, "apps", "memecoined"),
          windowsHide: true,
          stdio: ["ignore", "pipe", "pipe"],
          env: {
            ...process.env,
            ...(densityMode ? { MEMECOINED_VERIFY_DENSITY: "1" } : {}),
          },
        },
      );
      ...
      child.once("exit", (code) =>
        code === 0
          ...
          : reject(
              new Error(
                `MemeCoin'Ed ${densityMode ? "production-density" : "full-path"} verification exited with ${code}`,
              ),
            ),
      );
    });
  ...
  if (
    fullPath.diagnostic !== "discovery-to-displayed-paper-result" ||
    fullPath.controlledFixture !== true ||
    fullPath.verificationMode !== "supported-profiles" ||
    fullPath.profiles?.length !== 2 ||
    !fullPath.profiles.every(
      (profile) =>
        ["fast_furious", "oscillation_trader"].includes(profile.profile_id) &&
        Number(profile.buys) > 0 &&
        Number(profile.sells) > 0,
    ) ||
    ...
    productionDensity.profiles.some(
      (profile) => Number(profile.buys) === 0 || Number(profile.sells) === 0,
    ) ||
    ...
    Number(productionDensity.resetRegression?.preservedActivations ?? 0) !== 2
  ) {
    throw new Error(
      "MemeCoin'Ed full-path verification returned incomplete evidence",
    );
  }
  return {
    passedAt: new Date().toISOString(),
    commands: [
      "node dist/scripts/diagnose-profile-roundtrips.js",
...
const memecoinedVerification =
  !selectedBundles || selectedBundles.has("memecoined")
    ? await verifyMemecoinedPipeline()
    : null;
...
if (memecoinedVerification) {
  ...
  if (!bundle)
    throw new Error("MemeCoin'Ed bundle is missing after verification");
  await writeFile(
    join(output, `memecoined-verification-${releaseVersion}.json`),
    `${JSON.stringify(
      {
        schemaVersion: 1,
        releaseVersion,
...
process.stdout.write(
  `Created ${bundles.length} verified update bundles in ${output}\n`,
);
