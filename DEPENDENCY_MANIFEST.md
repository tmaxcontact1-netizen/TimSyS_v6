...
- No ORM, web framework, job-queue framework, dependency-injection framework or frontend dependency is approved.
...
- `express`, `fastify`, `koa`: not approved. If Telegram webhook mode is later selected, the schema must first approve the minimal native HTTP server contract.
