...
| `/src/entrypoints/dashboard.ts`         | TypeScript | Serves local paper observations plus authenticated, origin-checked watchlist and inert trading-configuration mutations; holds no trading authority | Runtime config; database readiness; paper reporting and dashboard persistence | Operator-started local dashboard process | Node.js native HTTP server                       |
