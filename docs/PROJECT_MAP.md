...
| `/frontend`           | Dependency-free local paper dashboard assets                                       |
...
| `/src/infrastructure/database/paper-dashboard.ts` | TypeScript | Reads bounded paper portfolio, token lifecycle, and realized book-equity history snapshots | Database pool; paper schema                                   | Local dashboard entrypoint                     | PostgreSQL client                     |
...
| `/src/entrypoints/dashboard.ts`         | TypeScript | Serves local read-only paper summary, detail, fixed-range performance, and validated token-lifecycle snapshots | Runtime config; database readiness; paper reporting and detail projections | Operator-started local dashboard process | Node.js native HTTP server                       |
