...
| `/src/infrastructure/database/paper-dashboard.ts` | TypeScript | Reads bounded paper positions, fills, realized results, and exit events in one snapshot | Database pool; paper schema                                   | Local dashboard entrypoint                     | PostgreSQL client                     |
...
| `/src/entrypoints/dashboard.ts`         | TypeScript | Serves local read-only paper summary/detail snapshots and dashboard assets                                | Runtime config; database readiness; paper reporting and detail projection  | Operator-started local dashboard process | Node.js native HTTP server                       |
