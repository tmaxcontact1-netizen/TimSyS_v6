...
| `/src/infrastructure/database/paper-dashboard.ts` | TypeScript | Reads bounded paper portfolio details and token-specific lifecycle snapshots    | Database pool; paper schema                                   | Local dashboard entrypoint                     | PostgreSQL client                     |
...
| `/src/entrypoints/dashboard.ts`         | TypeScript | Serves local read-only paper summary, detail, and validated token-lifecycle snapshots                     | Runtime config; database readiness; paper reporting and detail projections | Operator-started local dashboard process | Node.js native HTTP server                       |
