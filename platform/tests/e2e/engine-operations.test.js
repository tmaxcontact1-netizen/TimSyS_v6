...
test('backfills declared sources with an explicit boundary and runs governed positive providers',async function(){server=await helper.createTestServer('engine_operations');var login=await server.makeRequest('POST','/api/auth/dev-login',{}),token=login.data.token;
 ...
 var providers=await server.makeRequest('GET','/intelligence/providers',null,token);expect(providers.data.providers).toHaveLength(5);expect(providers.data.providers.map(function(p){return p.id;})).toContain('cover.operations');expect(providers.data.providers.every(function(p){return p.governance&&p.governance.confidenceMethod;})).toBe(true);
