...
test('reports healthy contracts and visibly detects an unevidenced product',async function(){server=await helper.createTestServer('intelligence_health');var login=await server.makeRequest('POST','/api/auth/dev-login',{}),token=login.data.token;
 var healthy=await server.makeRequest('GET','/intelligence/health',null,token);expect(healthy.status).toBe(200);expect(healthy.data.health.status).toBe('healthy');expect(healthy.data.health.checks.find(function(c){return c.name==='provider_scheduling';}).detail.registered).toBe(7);
